...
switch(_state)
{
	case SHOP_STATE.MENU:
		draw_sprite_stretched(spr_battle_soul_red,0,menu_divide_x + 20,border_y1 + 16 + 20*_index,8,8);
		break;
	case SHOP_STATE.BUY:
		if(_choice_state == 1)
			draw_sprite_stretched(spr_battle_soul_red,0,menu_divide_x + 12,border_y1 + 50 + 17*_index_buy,8,8);
		else
			draw_sprite_stretched(spr_battle_soul_red,0,border_x1 + 12,border_y1 + 16 + 20*_index,8,8);
		break;
	case SHOP_STATE.SELL:
		...
		if(_choice_state == 0)
		{
			...
			Y = border_y1 + 16 + 20*(_index div 2);
		}
		else
		{
