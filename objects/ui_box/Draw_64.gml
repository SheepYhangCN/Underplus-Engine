if(_state==0){
	...
	repeat(8){
		if(!Item_IsValid(Item_Get(proc))){
			draw_sprite_ext(spr_pixel,0,16+6+58,16+3+71+32*proc,180,1,0,c_red,1);
		}
		proc+=1;
	}
	proc=0;
	repeat(10){
		if(!Item_IsValid(Box_Get(box_slot,proc))){
			draw_sprite_ext(spr_pixel,0,16+6+58+302,16+3+71+32*proc,180,1,0,c_red,1);
		}
		proc+=1;
	}
	draw_sprite(spr_battle_soul_red,0,16+6+27+302*_choice_mode,16+3+69+32*_choice_item);
}