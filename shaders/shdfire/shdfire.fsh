...
void main() {
    ...
    vec3 color = vec3(1.0/(pow(abs(c+1.61),vec3(4.0))) * cos(shift * gl_FragCoord.y / iResolution.y));
    
    color=vec3(1.0,.2,.05)/(pow(abs((r.y+r.y)* max(.0,p.y)+0.1), 4.0));;
    color += (texture2D(gm_BaseTexture,uv*0.6+vec2(.5,.1)).xyz*0.01*pow(abs((r.y+r.y)*.65),5.0)+0.055)*mix( vec3(.9,.4,.3),vec3(.7,.5,.2), uv.y);
